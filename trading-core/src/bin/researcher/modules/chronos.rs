...
impl Chronos {
    ...
    pub fn observe_potential_hit(
        ...
        if current_sym_score < 0.001 { return false; }
        ...
        if is_triggering {
            if let Some(peak) = self.active_peaks.get_mut(symbol) {
                ...
                self.active_peaks.insert(symbol.to_string(), PeakCandidate {
    ...
    pub fn update_and_flush(
        ...
        if let Some(records) = self.pending_records.get_mut(symbol) {
            ...
            for r in records.iter_mut() {
                ...
                let calc_ret = |p_s: f64, p_n: f64| if p_s <= 0.0 { 0.0 } else { ((p_n - p_s) / p_s) * 100.0 };

                if r.ret_3s.is_none() && elapsed >= 3 { r.ret_3s = Some(calc_ret(p0, current_price)); }
                if r.ret_5s.is_none() && elapsed >= 5 { r.ret_5s = Some(calc_ret(p0, current_price)); }
                if r.ret_8s.is_none() && elapsed >= 8 { r.ret_8s = Some(calc_ret(p0, current_price)); }
                if r.ret_13s.is_none() && elapsed >= 13 { r.ret_13s = Some(calc_ret(p0, current_price)); }
                if r.ret_21s.is_none() && elapsed >= 21 { 
                    r.ret_21s = Some(calc_ret(p0, current_price));
                    r.z_entropy_21s = z_21.0; r.z_pressure_21s = z_21.1; r.z_nrg_21s = z_21.2;
                }
                if r.ret_34s.is_none() && elapsed >= 34 { 
                    r.ret_34s = Some(calc_ret(p0, current_price));
                    r.z_entropy_34s = z_34.0; r.z_pressure_34s = z_34.1; r.z_nrg_34s = z_34.2;
                }
            ...
            records.retain(|r| {
                if r.is_complete { completed.push(r.clone()); false } else { true }
            });
        }
        completed
    }
