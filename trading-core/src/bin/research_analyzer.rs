...
fn print_report(
    ...
    println!(
        "Processed Samples: {} | Efficiency: {:.1}%",
        proc,
        (proc as f64 / total as f64) * 100.0
    );
    println!("{}", separator);
    ...
    println!(
        "{:<15} | {:<10} | {:<10} | {:<10} | {:<10}",
        "REGIME", "SAMPLES", "WINRATE", "AVG RET", "EXPECTANCY"
    );
    for (name, s) in reg {
    ...
    for (name, s) in sym {
        println!(
            "{:<10} | Samples: {:<8} | Max Upside: {:>8.4}% | Max Drawdown: {:>8.4}%",
            name,
            s.count,
            s.max_upside * 100.0,
    ...
    println!(
        "{:<10} | {:<10} | {:<10} | {:<10} | {:<10}",
        "NRG BUCKET", "VBI ZONE", "SAMPLES", "WINRATE", "SIGNAL"
    );
