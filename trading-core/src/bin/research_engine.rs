...
impl ThermodynamicPhysicist {
    async fn new() -> anyhow::Result<Self> {
        ...
        if !file_exists {
            ...
            temp_writer
                .write_all(ValidationRecord::csv_header().as_bytes())
        ...
        if let Some(mut records) = self.validation_queue.get_mut(symbol) {
            ...
            if !nrg_values_5s.is_empty() || !nrg_values_10s.is_empty() {
                let mut stats = self
                    .correlation_stats
                    .entry(symbol.to_string())
                    .or_insert_with(|| CorrelationStats {
                ...
                if !nrg_values_5s.is_empty() {
                    let (correlation_5s, samples_5s) =
                        self.calculate_pearson_correlation(&nrg_values_5s, &returns_5s);
                    let alpha = 0.1;
                    stats.nrg_5s_correlation =
                        alpha * correlation_5s + (1.0 - alpha) * stats.nrg_5s_correlation;
                    stats.nrg_5s_samples += samples_5s;
                }

                if !nrg_values_10s.is_empty() {
                    let (correlation_10s, _) =
                        self.calculate_pearson_correlation(&nrg_values_10s, &returns_10s);
                    let alpha = 0.1;
                    stats.nrg_10s_correlation =
                        alpha * correlation_10s + (1.0 - alpha) * stats.nrg_10s_correlation;
                }
    ...
    fn analyze(
        ...
        history: &[MarketState],
        snapshot: &L2Snapshot,
    ) -> Result<RegimeClassifier, String> {
        ...
        let (optimal_entropy, std_dev, slope, mean_entropy) = {
            ...
            let variance = cache.iter().map(|&x| (x - mean).powi(2)).sum::<f64>() / n;
        ...
        if history.len() >= 10 {
            ...
            if !recent_regimes.is_empty() {
                ...
                let same_regime_count = recent_regimes
                    .iter()
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut ws = HyperliquidWs::new(false)
        .await
