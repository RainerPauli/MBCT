...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in lines {
        ...
        if line_counter >= CHUNK_SIZE {
            chunk_idx += 1;
            println!(
                "\n✅ Chunk #{} verarbeitet ({} Mio Zeilen total).",
                chunk_idx,
...
fn process_chunk_end(
    idx: usize,
    global: &mut HashMap<String, DeepCoinProfile>,
    chunk: &mut HashMap<String, DeepCoinProfile>,
    start: Instant,
) {
    for (sym, c) in chunk.drain() {
        let g = global.entry(sym.clone()).or_insert(DeepCoinProfile {
            symbol: sym,
            ..Default::default()
        });
    ...
    if let Some(btc) = global.get("BTC") {
        ...
        println!(
            ">>> Snapshot BTC: Ent: {:.4}, Vola21: {:.6}, Eff: {:.4}",
            btc.avg_entropy / n,
            btc.vola_21s / n,
            btc.avg_pressure / btc.avg_nrg
        );
    }
