...
impl SignalerPhysicist {
    ...
    fn process_snapshot(&self, snapshot: &L2Snapshot) -> Option<(String, MarketRegime, f64)> {
        ...
        if hist.len() > HISTORY_SIZE { hist.remove(0); }
    ...
    fn extract_volumes(&self, snapshot: &L2Snapshot) -> (f64, f64) {
        ...
        if snapshot.levels.len() >= 2 {
            for l in &snapshot.levels[0] { bv += l.sz.parse::<f64>().unwrap_or(0.0); }
            for l in &snapshot.levels[1] { av += l.sz.parse::<f64>().unwrap_or(0.0); }
        }
        (bv, av)
    }
    ...
    fn calculate_nrg(&self, history: &[MarketState]) -> f64 {
        ...
        let dp = (last.pressure - prev.pressure).to_f64().unwrap_or(0.0).abs();
        dp * 1000.0 
    }
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let raw_bytes = fs::read(META_PATH).map_err(|e| anyhow::anyhow!("Meta-File nicht lesbar: {}", e))?;
    let content = std::str::from_utf8(if raw_bytes.starts_with(&[0xEF, 0xBB, 0xBF]) { &raw_bytes[3..] } else { &raw_bytes })?;
    
    let meta: HLMeta = serde_json::from_str(content)?;
    let active_coins: Vec<String> = meta.universe.into_iter()
        .filter(|a| !a.is_delisted)
        .map(|a| a.name)
        .collect();

    println!("✅ Universum stabil. Monitoring {} Assets.", active_coins.len());
    ...
    loop {
        if let Some(snapshot) = ws.next_snapshot().await {
            ...
            if let Some((symbol, regime, nrg)) = physicist.process_snapshot(&snapshot) {
                ...
                if regime == MarketRegime::Oscillatory && nrg > 30.0 {
                    ...
                    println!("[{}] 🎯 EXHAUSTION: {:<8} | NRG: {:.2} | Re-Entry in Habitat", ts, symbol, nrg);
                }
            }
        }
        ...
        if last_log.elapsed() > Duration::from_secs(10) {
            ...
            println!(
                ...
                ts, active_coins.len(), total_snapshots, top_nrg.0, top_nrg.1, top_nrg.2, signal_count
            );
