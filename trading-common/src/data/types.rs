...
/// The thermodynamic state of the market
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct MarketState {
    pub symbol: String,
    /// Temperature (T) = Current Price
    pub temperature: Decimal,
    /// Pressure (P) = Orderbook Density
    pub pressure: Decimal,
    /// Volume (V) = Spread
    pub volume_spread: Decimal,
    /// Entropy Level (S) = Disorder/Chaos
...
impl MarketState {
    /// Berechnet die potenzielle Energie der "gespannten Feder"
    pub fn spring_tension(&self) -> Decimal {
        if self.volume_spread == Decimal::ZERO { 
            return Decimal::ZERO; 
        }
        self.pressure / self.volume_spread
    }
    
    /// Klassifiziert den Markt-Zustand basierend auf der Spannung
    pub fn state_classification(&self) -> &'static str {
        let tension = self.spring_tension();
        
        if tension > Decimal::from(1000) {
            "CRITICAL_COMPRESSION"
        } else if tension > Decimal::from(500) {
            "HIGH_TENSION"
        } else if tension > Decimal::from(100) {
            "MODERATE_TENSION"
        } else {
            "LOW_TENSION"
        }
    ...
    /// Berechnet die Entropie der Liquiditätsverteilung
    /// Ein niedriger Wert bedeutet Fokus (Gefahr), ein hoher Wert Rauschen.
    pub fn calculate_entropy(levels: &[(Decimal, Decimal)]) -> f64 {
        let total_volume: f64 = levels.iter()
            .map(|(_, sz)| sz.to_f64().unwrap_or(0.0))
            .sum();

        if total_volume == 0.0 { return 0.0; }

        levels.iter()
            .map(|(_, sz)| sz.to_f64().unwrap_or(0.0) / total_volume)
            .filter(|&p| p > 0.0)
            .map(|p| -p * p.ln())
            .sum()
    }
